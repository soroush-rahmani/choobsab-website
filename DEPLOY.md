...
پروژه از «وب‌سرویس خدماتی/الگو» ملی پیامک پشتیبانی می‌کند.

**در پنل ملی‌پیامک (panels.melipayamak.com):**
1. منوی **«وب سرویس خدماتی» / «الگوها»** → **الگوی جدید بساز**:
   - برای کد ورود: مثلاً `کد ورود شما به چوب‌ساب: %TEXT%`
   - برای سفارش: مثلاً `کاربر %TEXT% گرامی، سفارش شما ثبت شد`
   - صبر کن تأیید شود (معمولاً سریع است) و **bodyId الگو** را بردار.
2. **کلید وب سرویس** و **شماره خط** را از پنل ببین.
...
> - اگر `BODY_ID_OTP` / `BODY_ID_ORDER` تنظیم شوند، از `SendByTemplate` با پارامترهای الگو استفاده می‌شود:
>   - الگوی **کد ورود**: یک پارامتر (کد) → «کد ورود شما به چوب‌ساب: %TEXT%»
>   - الگوی **سفارش**: سه پارامتر به ترتیب _[نام، شناسه سفارش، مبلغ]_ → مثلاً:
>     `%TEXT% گرامی، سفارش شما با شناسه %TEXT2% ثبت شد. مبلغ: %TEXT3% تومان`
> - اگر bodyId نباشد، حالت «ارسال ساده» (`/api/Send`) با متن‌های پیش‌فرض استفاده می‌شود (که نیازمند `MELIPAYAMAK_FROM` است).
